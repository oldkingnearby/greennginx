    ...
    <div class="register-wrapper"> 
      <div id="register">
        ...
        >
          ...
          <el-form-item>
            <el-button type="primary" @click="submitForm('ruleForm2')" style="width:100%;">重置密码</el-button>
            <p class="login" @click="gotoLogin">登录</p>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
<script> 
import { getEmailVertifyCode,requestResetpassword,requestCheckuseremail } from '../api/api';
  ...
  methods: {
    ...
    sendCode () {
      ...
      if (this.checkEmail(email)) {
        ...
        requestCheckuseremail({username:this.ruleForm2.username,email:this.ruleForm2.email}).then(data=>{
            let {code,msg}=data;
            if (code !== 200) {
                this.$message({
                  message: msg,
              ...
              {
                getEmailVertifyCode({email:this.ruleForm2.email}).then(res=>{
                    let {code,msg}=res;
                    if (code !== 200) {
                            this.$message({
                            message: msg,
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          requestResetpassword(rigesterParams).then(data=>{
            let {code,msg}=data;
            if (code !== 200) {
                  this.$message({
                    message: msg,
